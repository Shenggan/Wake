...
wol.state = {
  ...
  toWake: {
    ...
    setipBoardCast: function(v) {
      wol.state.toWake.ipBoardCast = v;
    },
  },
  success: {
    timeout: null,
  ...
  wake: function (device) {
    if (typeof device !== 'undefined') {
      ...
      wol.wakeDevice({name: wol.state.toWake.name,
                      ...
                      ipBoardCast: wol.state.toWake.ipBoardCast});
    }
  },
  ...
  add: function (device) {
    ...
    wol.state.toWake.setipBoardCast('');
    wol.state.error = {};
  },
...
wol.devicesView = function () {
  ...
  var firstRow = m('tr', [
    ...
    m('td', m('input[type=text]', {'form': form.attrs.id,
                                   onchange: m.withAttr('value', wol.state.toWake.setipBoardCast),
                                   value: wol.state.toWake.ipBoardCast,
                                   class: 'form-control',
                                   placeholder: 'IP Boardcast'})),
  ...
  var rows = wol.state.devices.map(function (device) {
    return m('tr', [
      ...
      m('td', m('code', device.ipBoardCast)),
      m('td',
  ...
  return [form,
          m('table.table', {class: ''},
            m('thead', m('tr', [
              ...
              m('th', {class: 'col-md-2'}, 'IP Boardcast'),
              m('th', {class: 'col-md-1'}),
              m('th', {class: 'col-md-1'})
            ])),
